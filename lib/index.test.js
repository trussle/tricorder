...
describe("index", () => {
  ...
  describe("metrics", () => {
    ...
    describe("http_requests", () => {
      test("Given metrics are running, should include http_requests", () => {
        // Arrange
        // Act
        return agent.get("/resource").then(() => {
          return agent.get("/metrics").then(result => {
            ...
            expect(result.text).toContain("http_requests 0");
          });
        });
      });
    });

    describe("http_requests_count", () => {
      test("Given resource has been request, should being included in http_requests_count", () => {
        // Arrange
        // Act
        return agent.get("/resource").then(() => {
          return agent.get("/metrics").then(result => {
            ...
            expect(result.text).toContain(
              'http_requests_count{method="GET",path="/resource",status="200"} 2'
            );
          });
        });
      });

      test("Given resource/:id has been request, should being included in http_requests_count", () => {
        // Arrange
        const id = 8;
        ...
        return agent.get(`/resource/${id}`).then(() => {
          return agent.get("/metrics").then(result => {
            ...
            expect(result.text).toContain(
              'http_requests_count{method="GET",path="/resource/:id",status="200"} 1'
            );
          });
        });
      });
